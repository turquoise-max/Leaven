-- Update generate_staff_schedules RPC to use schedule_type 'regular' and avoid hardcoded '정규 근무' title if not needed
      ...
      IF v_schedule IS NOT NULL THEN
        ...
        IF v_start_str IS NOT NULL AND v_end_str IS NOT NULL THEN
            ...
            IF NOT EXISTS (
                ...
                -- Insert Schedule (Set title to NULL so it can follow system defaults, and set schedule_type to 'regular')
                INSERT INTO schedules (store_id, title, start_time, end_time, color, memo, schedule_type)
                VALUES (p_store_id, NULL, v_start_ts, v_end_ts, v_staff.role_color, '패턴 기반 자동 생성', 'regular')
