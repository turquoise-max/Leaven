-- 1. Add member_id column to schedule_members
ALTER TABLE public.schedule_members ADD COLUMN member_id UUID REFERENCES public.store_members(id) ON DELETE CASCADE;

-- 2. Migrate existing data (map user_id to member_id)
UPDATE public.schedule_members sm
SET member_id = (
  SELECT m.id
  FROM public.store_members m
  JOIN public.schedules s ON s.id = sm.schedule_id
  WHERE m.user_id = sm.user_id AND m.store_id = s.store_id
  LIMIT 1
);

-- Delete orphans if any (where member_id couldn't be resolved)
DELETE FROM public.schedule_members WHERE member_id IS NULL;

-- 3. Modify columns and constraints
ALTER TABLE public.schedule_members ALTER COLUMN member_id SET NOT NULL;
ALTER TABLE public.schedule_members DROP CONSTRAINT IF EXISTS schedule_members_schedule_id_user_id_key;
ALTER TABLE public.schedule_members ADD UNIQUE(schedule_id, member_id);
ALTER TABLE public.schedule_members DROP COLUMN user_id;

-- 4. Recreate RLS policies for schedule_members
DROP POLICY IF EXISTS "Members can view schedule members in their store" ON public.schedule_members;
DROP POLICY IF EXISTS "Managers can manage schedule members" ON public.schedule_members;
...
CREATE POLICY "Managers can manage schedule members"
...
-- 5. Update generate_staff_schedules RPC
      ...
      IF v_schedule IS NOT NULL THEN
        ...
        IF v_start_str IS NOT NULL AND v_end_str IS NOT NULL THEN
            ...
            IF NOT EXISTS (
                ...
                VALUES (p_store_id, '자동 생성 근무', v_start_ts, v_end_ts, v_staff.role_color, '패턴 기반 자동 생성')
